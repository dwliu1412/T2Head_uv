...
class Head3DGSLKsRig(BaseLift3DSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 训练阶段不再计算 CLIP 文本-图像相似度
        # if self.clip_evaluator is not None:
        #     clip_imgs = images.permute(0, 3, 1, 2)
        #     if hasattr(prompt_utils, "prompt"):
        #         prompts = [prompt_utils.prompt]
        #     elif "text" in batch:
        #         prompts = [batch["text"]]
        #     else:
        #         prompts = ["a person head portrait"]
        #
        ...
        #         self.log(f"train/clip_sim_{tag}", sims.mean().item())
